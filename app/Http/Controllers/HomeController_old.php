...

{
  ...
  public function chefList()
  {
    ...
    //  $user=User::find('250');
    // return unserialize($user->available_dates);
...

  {
    ...
    if ($request->isMethod('post')) {
      ...
      if (isset($latLng) && !empty($latLng)) {
        ...
        $nearquery->select(DB::raw("users.*, menus.name, menus.ingredients, menus.description,  111.045 * DEGREES(ACOS(COS(RADIANS(" . @$latLng['lat'] . ")) * COS(RADIANS(users.latitude))  * COS(RADIANS(users.longitude) - RADIANS(" . @$latLng['lng'] . "))  + SIN(RADIANS(" . @$latLng['lat'] . ")) * SIN(RADIANS(users.latitude)))) AS distance"))
      ...
      return response()->json(['chefs' => $chefs, 'chefs_near' => $nearchefs, 'favs' => $favs]);
    }
  }
...

  {
    ...
    // for testing
    // $sent = Mail::to('navdeepkumar@gmail.com')->send(new Support($data));
