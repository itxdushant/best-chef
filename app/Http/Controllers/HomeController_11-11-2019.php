...
class HomeController extends Controller
{
    ...
    public function mainPage()
    {
        ...
        return view('main-page')->with([ "chefs" => $chefs, "randchefs" => $randchefs]);
    ...
    public function searchData(Request $request)
    {
        if($request->isMethod('post')) {
            ...
            if($request->filled('service_area')) {
                $query->where('service_area', 'LIKE', "%{$request->service_area}%");                
            }
            if($request->filled('available_dates')) {
                $query->where('available_dates', 'LIKE', "%{$request->available_dates}%");                
            }
            if($request->filled('meal_perference')) {
                $query->where('description', 'LIKE', "%{$request->meal_perference}%");                
            }
