...
<script type="text/javascript">
  $(document).ready(function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          //  let html = `<div class="gmap_canvas" style=""><iframe width="670" height="335" id="gmap_canvas" src="https://maps.google.com/maps?q=${chefs.service_area}&t=&z=13&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe></div>`;
            ...
            renderHtml(chefs_near, chefsWithReview, favs);
            $("#map").html("");
            window.initMap = initMap(data.pinLocations);
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          if (Object.keys(data.chefs_near).length) {
            ...
            renderHtml(chefs_near, chefsWithReview, favs);
            var locations = data.pinLocations;
    ...
    function renderHtml(chefs, chefsWithReview, favs) {
      let html = "";
      ...
      for (key in chefs) {
        ...
        chefs[key]['rating'] = 0;
        var review = 0;
        var chefsWithReviewIds = [];
        if (chefsWithReview[key] !== undefined) {
          $.each(chefsWithReview, function(index, value) {
            if (value.id == chefs[key].id) {
              chefs[key]['review'] = value.chef_reviews;
              chefs[key]['rating'] = (value.rating)? value.rating: 0;
            }
          });
        }
        html += `  
       ...
       // console.log(chefs[key].meal_prefrences, "dsdsds");
        if( chefs[key].meal_prefrences !== undefined){
        ...
        html += ` </div>
              ...
              <div class="bottom_stars">
               <p><img src="images/g_star.png"> `+ chefs[key]['rating'] +` <span>(` + chefs[key]['review'] + ` reviews)</span></p>
               <div>
      ...
      $("#chefs-list").html(html);
      $('.remove-to-fav').prop('checked',true);
...
<script defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAQKT-baFr1GJfDvhCjhN2HLMxmyNbTcys&callback=initMap" >
</script>

<script>
// Initialize and add the map
async function initMap(initMap) {
  console.log(initMap, "testing");
