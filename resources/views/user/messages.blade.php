					...
					<div class="chat_box_grid">
						...
						<div class="message-submit-box msg_div">
						  <span class="form-group comment-grid">
							...
							<input type="text" class="form-control message-box" id="message" aria-describedby="text" name="message" placeholder="" required="">
							<input type="hidden" name="gid" id="gid" value="<?php if(isset($_GET['g'])) { echo $_GET['g']; } ?>">                                                 
						  </span>
						  <span class="submit-message-grid"><button type="button" name="sub" id="send_msg"><i class="fa fa-send-o"></i></button></span>
						</div>
					</div>
				</div>
					
				<div class="col-md-12 col-lg-4 user-msg-sidebar">
					<h3 class="title-heading-small">Choose a Chef</h3>                                           
					@foreach ($users as $user)
					<div class="user-msg-grid">                      
						@if($user->profile_pic)
						  <img width="50" height="50" class="img-fluid mr-2 user-img{{$user->id}}" src="{{asset('public/uploads/profiles')}}/{{$user->profile_pic}}" onerror="this.onerror=null;this.src='{{asset('public/img/default-user.png')}}';"/>
						@else
						  <img width="50" height="50" class="img-fluid mr-2 user-img{{$user->id}}" src="{{asset('public/img/default-user.png')}}" />
						@endif
    ...
    $(document).ready(function () {
        ...
        function sendMsg() {
          ...
          if(message != "") {
              $.ajax({
                 type:'POST',
                 url:'{{url("user/send-msg")}}',
                 data: { 'uid': uid, 'message': linkify(message)},
                 success:function(data) {
                    $("#message").val('')
                    loadmsgs(uid);                                    
                 }
                            ...
                            if(messageData[key].sender == uid) {
                              ...
                              html += `<div class="activity-row activity-row1 ${dclass}">
								<div class="outer_msg_activity clearfix">
                                  ...
                                  <div class="activity-img"><img src="${img}" width='60px' height='60px' class="img-responsive ${imgCls}" alt="user pic"><span>${time}</span></div>
                              ...
                              html += `<div class="activity-row activity-row1 ${dclass}">
								<div class="income_msg_activity clearfix">
                                  <div class="activity-img">
									<img src="${img}" width='60px' height='60px' class="img-responsive ${imgCls}" alt="user pic"><span>${time}</span>
								  </div>
