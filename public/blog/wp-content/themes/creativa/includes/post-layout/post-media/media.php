...
<div class="post-media">
  ...
  <?php 
    ...
    if (is_single()) {
      ...
      if (class_exists('Aq_Resize') && $image_src[1] >= $image_w && $image_src[2] >= $image_h) {
    ...
    if ($image) {
      ...
      echo '<img src="'. esc_url($img_url) .'" width="'. esc_attr($image[1]) .'" alt="'. esc_attr__('Post Image', 'creativa') .'" '. creativa_retina_check($image[0], true) .'  />';
      if (!is_single()) { echo '</a>'; }
    }
  }
