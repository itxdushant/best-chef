...
if (has_post_thumbnail() && !post_password_required() && $creativa_options['opt-show-thumbnail'] == true) {
  ...
  if ($standard_media_style == 3 && $allow_media_styles == 1) {
    ...
    $creativa_image_th[0] = creativa_thumbnail_size(0, 600);
    $creativa_image_th[1] = creativa_thumbnail_size(1, 500);
  }
} else {
  $blog_media_class = 'blog-media-standard ';
...
<?php 
  ...
  $blog_sidebar = $creativa_options['opt-show-sidebar'];
  $col_class = 'col-md-6';
...
<div class="<?php echo esc_attr($col_class) ?> masonry-post-wrap">

<!-- Blog Post -->
<article id="post-<?php the_ID(); ?>" <?php post_class($hpth . $blog_media_class); ?>>
